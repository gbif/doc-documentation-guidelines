...
Some documents are published as multiple versions.  This is done using _branches_ in Git: the name of the branch, such as `1.0` or `2019`, is the identifier for the version.  This allows for edits to old versions, such as updating a link or making correcting a syntax error in the document.

// For multi-version projects, Jenkins is set to build from every branch, and deploy to a subdirectory on the webserver.

=== AsciiDoctor

GBIF documentation is maintained and published using a fast text processor and publishing toolchain called _AsciiDoctor_.  AsciiDoctor converts the AsciiDoc syntax into the required formatting depending on the output format.

The document (or parts of it) is stored in files with the extension `.en.adoc`, where `.en.` indicates the language using ISO two-letter language codes. Other source files such as images have their usual extensions.

The main document that compiles any given document is called `index.en.adoc`.

=== Generating the document

The source code in the repository is converted into the finished HTML and PDF documents using the _Asciidoctor_ tool.  Every time a change is made to the repository, the GBIF build server is notified.  It retrieves the document source code, generates the document, and copies the formatted documents to a webserver.
...
If you are familiar with software development tools you can build a document on your own computer - this is useful for previewing changes.  You will first need to setup https://www.docker.com/[Docker].  You can then build the HTML document with this command, run from the top level directory:

`+docker run --rm -it --user $(id -u):$(id -g) -v $PWD:/documents/ gbif/asciidoctor-toolkit+`

The resulting documents are in subdirectories coded by language (such as `en`), including both HTML and PDF files.
...
* Set a `payload` parameter to receive the GitHub information
* *Source Code Management*: Under advanced Git settings, set the branches to build to `origin/*` and `Check out to specific local branch` to `+**+`
* *Build Triggers*: Set an authentication token
*  Set a build script, which varies depending whether the document is keeping old versions deployed.

==== GitHub setup

* Set up a new webhook, with the path e.g. `http://builds.gbif.org/job/doc-documentation-guidelines/buildWithParameters?token=XXXXXXXXXX` (with the token from above)
