...
The source code is written in a computer language called _AsciiDoctor_, a type of _markup language_.  This means the format and structure of the document (such as headings and bold text) are applied using particular symbols visible in the text, and are converted to the required formatting when the document later.
